...
// checando se houve ou não colisão com o jogador, além de deletá-los
  update(dt, player, enemies) {
    ...
    Enemy.deleteEnemies(enemies);    

  }

// Caso o inimigo saia da tela é deletado da array
  static deleteEnemies(enemies){
...
// Adiciona inimigos na array, com características de acordo com o level   
  static addEnemies(enemies, player){
...
// Adiciona na tela os primeiros inimigos
  static addFirstEnemies(enemies, player){
    for (let i = 1; i <= 4; i++) {
        enemies.push(new Enemy(
...
class WinFlag extends Component {
  ...
  // Adiciona bandeiras na tela
    static addWinFlags(x, y, sprite, col, flags){
  ...
  // Executa os métodos relacionados à vitória de um nível
    static flagExists(col, x, y, flags, player){
        if(WinFlag.flagStatus(col, flags)){
            WinFlag.addWinFlags(x, y, 'images/rock.png', col, flags);
            player.spawnPlayer();
...
// checar se houve vitória ou derrota
  update(flags) {
    if (this.y === spawnY[0]){
        switch(this.x){
            case(spawnX[0]):
                WinFlag.flagExists(spawnX[0], this.x, this.y, flags, this)   
                break;  
            case(spawnX[1]):
                WinFlag.flagExists(spawnX[1], this.x, this.y, flags, this) 
                break;  
            case(spawnX[2]):
                WinFlag.flagExists(spawnX[2], this.x, this.y, flags, this)  
                break;  
            case(spawnX[3]):
                WinFlag.flagExists(spawnX[3], this.x, this.y, flags, this)  
                break;  
            case(spawnX[4]):
                WinFlag.flagExists(spawnX[4], this.x, this.y, flags, this)  
                break;                                                                  
        }                   
    }        
    else{
...
// Aumenta o nível do jogo, alterando suas características
  increaseLevel(){
    ...
    enemiesInterval = setInterval( () => {Enemy.addEnemies(allEnemies, this)},levelInfo[this.level][2]);
    bugType++;
  }
...
// verificando se são válidos
  handleInput(value, flags, enemies) {
    if(this.freezeControls()){    
        switch(value){
          case('up'):
              if (this.y === spawnY[1] && WinFlag.flagStatus(this.x, flags) === false)
                break;  
...
// Reinicia a partida, zerando os dados da anterior
  restart(enemies) {
    ...
    timer = Date.now()    
    bugType = 0;    
    Enemy.addFirstEnemies(enemies, this);    
    clearInterval(enemiesInterval);    
    enemiesInterval = setInterval( () => {Enemy.addEnemies(enemies, this)},levelInfo[this.level][2]);
  }
...
let enemiesInterval = setInterval( () => {Enemy.addEnemies(allEnemies,player)},levelInfo[player.level][2]);

// Instanciar os primeiros inimigos 
Enemy.addFirstEnemies(allEnemies, player);
